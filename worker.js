...
async function handleCheckinInit(request, env) {
  ...
  const alreadyChecked = await env.DB.prepare('SELECT 1 FROM checkins WHERE device_id = ? AND tag_uid = ? AND checkin_date = ? AND check_type = ? LIMIT 1').bind(deviceId, uid, dateStr, ct.type).first();
  if (alreadyChecked) {
    await audit(env, 'code_reuse', `设备已打卡,尝试复用 code UID=${uid} 类型=${ct.type}`, deviceId, getClientIp(request));
    return json({ error: '该设备今日已在此科室打过此类型卡，请勿重复打卡，如需打卡请重新触碰 NFC 标签' }, 409, env);
  }
